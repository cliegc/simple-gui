...
static void ShowFrameRate() {
    auto fpsLbl = SG_GuiManager.GetWindow().AddComponent<Label>("");
    ...
    auto deltaLbl = SG_GuiManager.GetWindow().AddComponent<Label>("");
    ...
    btn2->clicked.Connect("on_clicked",
                          [timer]() {
                              if (timer)
                                  ...
                                  SG_INFO("btn clicked: delete timer");
                          });
...
static void ViewImage() {
    class ClickToTop final : public ExtendedFunctions {
    protected:
        bool HandleEvent(Event *event) override {
            if (auto ev = event->Convert<MouseButtonEvent>();
                ev && ev->IsPressed(MouseButton::Left)) {
                if (!m_target->GetGlobalRect().ContainPoint(ev->GetPosition())) return false;

                auto &children = m_target->GetParent()->GetChildren();
                if (children[children.size() - 1].get() == m_target) return false;

                auto rit = std::find_if(children.rbegin(), children.rend(),
                                        [this](auto &child) {
                                            return child.get() == this->m_target;
                                        });
    ...
    class OpenDroppedFile final : public ExtendedFunctions {
    protected:
        bool HandleEvent(Event *event) override {
            if (const auto ev = event->Convert<DropEvent>();
                ev && ev->IsDropFile()) {
                std::string path = ev->GetContent();
                auto texture = SG_GuiManager.GetWindow().GetRenderer().CreateSharedTexture(path);
                if (texture->IsNull()) {
                    SG_INFO("Texture could not be loaded: {}", path);
                    return false;
                }

                auto dp = SG_GuiManager.GetWindow().AddComponentDeferred<DraggablePanel>(path);
                auto textureRect = dp->AddChildDeferred<TextureRect>(texture);
                ...
                dp->SetSize(300, 300);
                dp->SetGlobalPosition(ev->GetPosition() - dp->GetSize() / 2);
                dp->SetGlobalDragEnable(true);
                dp->AddExtendedFunctions<ClickToTop>();

                dp->SetToolTipEnabled(true);
                auto tipLbl = dp->SetToolTip<Label>(path);
                tipLbl->CustomThemeColor(ThemeColorFlags::LabelBackground, Color(0, 0, 0, 200));


                SG_INFO("Open image file: {}", path);
                return true;
            }
    ...
    auto lbl = SG_GuiManager.GetWindow().AddComponent<Label>("Drag and drop the image file to this window to open it");
    lbl->SetSizeConfigs(ComponentSizeConfig::Expanding, ComponentSizeConfig::Expanding);
    lbl->SetTextAlignments(TextAlignment::Center, TextAlignment::Center);
    lbl->CustomThemeColor(ThemeColorFlags::LabelForeground, Color(175, 175, 175));
    lbl->GetFont().SetSize(18);
    lbl->GetFont().SetStyle(FontStyle::Bold);
